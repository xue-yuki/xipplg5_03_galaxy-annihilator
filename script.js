// --- Inisialisasi Elemen ---
        ...
        // --- Variabel State Game ---
        let player, bullets, enemies, stars, particles, planets, powerUps;
        ...
        // PENAMBAHAN: Power-up
        const createPowerUp = () => ({ x: Math.random() * (canvas.width - 30), y: -30, size: 15, speed: 2, color: '#0f0' });

        // --- Fungsi Menggambar ---
        const drawPlayer = () => {
            const p = player;
            const powerUpColor = 'rgba(255, 223, 0, 0.9)';
            ctx.save();
            // Efek pendorong
            const thrusterHeight = Math.random() * 20 + 15;
            ctx.fillStyle = p.isPoweredUp ? powerUpColor : `rgba(255, 180, 0, ${Math.random() * 0.5 + 0.5})`;
            ctx.shadowColor = p.isPoweredUp ? powerUpColor : '#ff0';
            ctx.shadowBlur = 20;
            ctx.beginPath(); ctx.moveTo(p.x + p.width * 0.3, p.y + p.height * 0.8); ctx.lineTo(p.x + p.width * 0.7, p.y + p.height * 0.8); ctx.lineTo(p.x + p.width / 2, p.y + p.height * 0.8 + thrusterHeight); ctx.closePath(); ctx.fill();
            // Badan pesawat
            ctx.fillStyle = '#ccc'; ctx.shadowColor = p.color; ctx.shadowBlur = 15; ctx.beginPath(); ctx.moveTo(p.x + p.width / 2, p.y); ctx.lineTo(p.x, p.y + p.height * 0.8); ctx.lineTo(p.x + p.width, p.y + p.height * 0.8); ctx.closePath(); ctx.fill();
            // Kokpit
            ctx.fillStyle = p.isPoweredUp ? powerUpColor : p.color; ctx.beginPath(); ctx.arc(p.x + p.width / 2, p.y + p.height * 0.4, p.width * 0.15, 0, Math.PI * 2); ctx.fill();
            ctx.restore();
        };
        ...
        const drawPowerUps = () => { powerUps.forEach(p => { ctx.save(); ctx.fillStyle = p.color; ctx.shadowColor = p.color; ctx.shadowBlur = 20; const angle = Date.now() / 200; for(let i=0; i<5; i++){ ctx.lineTo(p.x + p.size * Math.cos(angle + i*2*Math.PI/5), p.y + p.size * Math.sin(angle + i*2*Math.PI/5)); } ctx.closePath(); ctx.fill(); ctx.restore(); }); };
        ...
        function checkCollisions() {
            ...
            // Pemain vs Musuh
            for (let i = enemies.length - 1; i >= 0; i--) { const e = enemies[i]; if (player.x < e.x + e.width && player.x + player.width > e.x && player.y < e.y + e.height && player.y + player.height > e.y) { playerHitSound.triggerAttackRelease("0.3n"); createExplosion(player.x + player.width / 2, player.y + player.height / 2, player.color); enemies.splice(i, 1); lives--; } }
            // Pemain vs Power-up
            for (let i = powerUps.length - 1; i >= 0; i--) { const p = powerUps[i]; if (player.x < p.x + p.size && player.x + player.width > p.x && player.y < p.y + p.size && player.y + player.height > p.y) { powerUpSound.triggerAttackRelease("C4", "4n"); player.isPoweredUp = true; player.powerUpTimer = POWERUP_DURATION; powerUps.splice(i, 1); } }
        }
        ...
        function gameLoop() {
            ...
            drawStars(); drawPlanets();
            drawPlayer();
            bullets.forEach(drawBullet); enemies.forEach(drawEnemy); particles.forEach(drawParticles); powerUps.forEach(drawPowerUps);
        ...
        window.addEventListener('keydown', (e) => {
            keys[e.code] = true;
            if (e.code === 'Space' && !gameOver && !keys.spacePressed) {
                shootSound.triggerAttackRelease("C5", "32n");
                if (player.isPoweredUp) {
                    bullets.push(createBullet(player.x + player.width / 2 - 3, player.y, 0)); // Tengah
                    bullets.push(createBullet(player.x + player.width / 2 - 3, player.y, -0.2)); // Kiri
                    bullets.push(createBullet(player.x + player.width / 2 - 3, player.y, 0.2)); // Kanan
                } else {
                    bullets.push(createBullet(player.x + player.width / 2 - 3, player.y));
                }
                keys.spacePressed = true;
            }
        });
        window.addEventListener('keyup', (e) => { keys[e.code] = false; if (e.code === 'Space') keys.spacePressed = false; });
        startButton.addEventListener('click', () => { if (Tone.context.state !== 'running') Tone.start(); startGame(); });
        restartButton.addEventListener('click', startGame);
        
        // --- Inisialisasi Awal ---
        window.onload = () => { stars = createStars(); prepareGame(); ctx.clearRect(0, 0, canvas.width, canvas.height); drawStars(); drawPlanets(); drawPlayer(); startScreen.classList.remove('hidden'); };